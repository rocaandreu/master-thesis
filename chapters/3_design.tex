...
Based on the context above, the design must satisfy the following requirements:
\begin{itemize}
  \item Throughput of 100 Gb/s to feed Ethernet MAC
  \item Latency of under 10 $\mu$s to let the \gls{fec} encoder dominate
  \item Configurable size (from 8 to 64)
  \item Configurable number of flows
  \item Configurable generation size
  \item Configurable queue length (both free memory and flow queues)
\end{itemize}

\subsection{Throughput and latency}
Of the requirements above, throughput and latency are the two that drive the architecture the
most directly: the buffer must sustain 100 Gbit/s to match the Ethernet
\gls{cmac} it is fed from, and it must add no more than 10 $\mu$s of latency so
that the \gls{fec} encoder's own processing time dominates the end-to-end delay budget.\\
...
The design presented in this chapter is validated exclusively through simulation; no
...
target workload. $<$TODO: CITE CONC FLOW CNT$>$ count Based on measurements reported in the literature, 
on the order of 30 to 40 flows can be expected to require concurrent \gls{fec} protection at any given time in a
representative deployment. Using this as a start reference point, the example configuration used
throughout the rest of this thesis is deliberately chosen on the larger side, in order to
stress the design: \\
...
drained by the read engine, without stalling ingest. With 128 supported flows against an
expected 30 to 40 concurrent flows, the configuration leaves considerable headroom. \\
