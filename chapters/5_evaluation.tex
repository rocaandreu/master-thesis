...
are fed into the buffer with the generation size configured to four. The four packets are
written in order and are eventually forwarded to the encoder together, once the full
...
the free memory queue is popped -- and the corresponding \gls{axi} write requests to \gls{hbm}
begin -- before the packet has fully arrived at the write engine, rather than only after the
...
The design is evaluated using an almost UVM-like testbench, built to allow controlled,
repeatable stimulus generation alongside fine-grained monitoring of the datapath. It consists
of three main components. A behavioral \gls{hbm} memory model stands in for the physical
\gls{hbm} subsystem, with independently configurable response latency and in-flight
transaction depth for both read and write requests, allowing the buffer to be exercised under
different memory-timing assumptions. A UDP packet generator drives the ingress side of the
design; it supports parametrizable packet length as well as configurable interleaving between
flows, letting the number of flows, the number of packets sent per flow, and the interleaving
pattern between them all be chosen per test. Finally, a simple \gls{fec} encoder model sits on
the egress side, and simply consumes whatever data the buffer forwards to it, without
performing any encoding itself. All three components -- the packet generator, the \gls{hbm}
model, and the encoder model -- communicate with the design under test over the same
\gls{axi}-Stream interfaces described in Chapter~\ref{ch:implementation}.
...
arrive affects latency. Finally, the configuration sweep  evaluates how scaling the
design's parameters -- generation size, flow count, and queue depths -- together affects both
...
100 Gb/s corresponds to a theoretical delay of approximately
970 $\mu$s.
