...
The design was evaluated purely through simulation, using a testbench built around a
...
As discussed in chapter \ref{ch:discussion}, average and worst-case packet latency were shown
...
At the same time, the design presented here is closer to baseline rather than a
deployment-ready module. It has only been verified in simulation, its parameters are fixed at
elaboration time rather than being runtime-configurable, and its post-synthesis resource usage
is dominated by flip-flop-based FIFOs rather than dedicated on-chip memory. \\

As it is already covered in chapter \ref{ch:discussion}, 
the directions for addressing the aforementioned these points would be 
...
set of next steps for turning that baseline into a production-ready component of a future
SmartNIC transmit pipeline.
